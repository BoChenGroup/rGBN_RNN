...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 12 23:15:40 2018
...
class MyThread(threading.Thread):
    def __init__(self, count,X_train,Phi, Theta1, L, K, T, Pi):
    ...
    def run(self):
            ...
            for l in range(self.L):
                ...
                if l == 0:
                    [A_KT[l],  self.A_VK[l]] = PGBN_sampler.Multrnd_Matrix(self.X_train, self.Phi[l], self.Theta[l])
                else:
                    [A_KT[l], self.A_VK[l]] = PGBN_sampler.Multrnd_Matrix(Xt_to_t1[l],  self.Phi[l],  self.Theta[l])

                if l ==  self.L - 1:
                    for t in range( self.T - 1, 0, -1):  # T-1 : 1
                        tmp1 = A_KT[l][:, t:t + 1] + L_dotkt[l][:, t + 1:t + 2]
                        tmp2 = cf.tao0 * np.dot(self.Pi[l], self.Theta[l][:, t - 1:t])
                        L_kdott[l][:, t:t + 1] = PGBN_sampler.Crt_Matrix(np.array(tmp1, order='C').astype('double'),
                                                                         np.array(tmp2, order='C').astype('double'))

                        [L_dotkt[l][:, t:t + 1], tmp] = PGBN_sampler.Multrnd_Matrix(np.array(L_kdott[l][:, t:t + 1], dtype=np.double, order='C'),  self.Pi[l],
                    ...
                    for t in range(self.T - 1, 0, -1):
                        ...
                        L_kdott[l][:, t:t + 1] = PGBN_sampler.Crt_Matrix(np.array(A_KT[l][:, t:t + 1] + L_dotkt[l][:, t + 1:t + 2], order='C').astype('double'),
                        ...
                        [tmp, X_layer[l][:, t, :]] = PGBN_sampler.Multrnd_Matrix(tmp_input,
                        ...
                        [L_dotkt[l][:, t:t + 1], tmp] = PGBN_sampler.Multrnd_Matrix(tmp_input,  self.Pi[l],
                                                                                    np.array( self.Theta[l][:, t - 1:t],
                                                                                             order='C'))
                        self.L_KK[l] =  self.L_KK[l] + tmp

                    L_kdott[l][:, 0:1] = PGBN_sampler.Crt_Matrix(A_KT[l][:, 0:1] + L_dotkt[l][:, 1:2],
