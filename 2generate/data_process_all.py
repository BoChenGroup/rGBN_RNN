...
def get_batch_tm_lm_test(Doc, V, tm_ignore, Batch_Size, pad_id = 0 ):
    x, y, d, m = [], [], [], []
    Doc_len = [len(Doc[id].copy()) for id in range(Batch_Size)]
    max_sent_len = cf.max_sent_each_doc
    for idx_sent in range(Batch_Size):
        xx, yy, dd, mm = [], [], [], []
        for s in range(max_sent_len-Doc_len[idx_sent]):
            Doc[idx_sent].append((Doc[idx_sent][0][0], len(Doc[idx_sent]) + s, [pad_id] * cf.lm_sent_len))
            ######## pading 0 of sentence to the doc which is less than max sentences
        s_pre_bow = np.zeros([V-len(tm_ignore)])
        for docid, seqid, seq in Doc[idx_sent]:
            xx.append(pad(seq[:-1], cf.lm_sent_len, pad_id))
            yy.append(pad(seq[1:], cf.lm_sent_len, pad_id))
            dd.append(s_pre_bow.copy())
            s_pre_bow += Bow_sents(seq,V,tm_ignore)
        ...
        d.append(dd)
        m.append(mm)
    return  np.array(x), np.array(y), np.array(d), np.array(m), max_sent_len