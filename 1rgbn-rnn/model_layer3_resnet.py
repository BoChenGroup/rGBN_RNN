...
#######################################DPGDS_PARAM###############################################
        ...
        self.b_5_k, self.W_5_l, self.b_5_l, self.Wres_1, self.Wres_2 = self.initialize_weight()
...
#######################################LSTM_PARAM###############################################
        ...
        for j in range(config.sent_J):
            ...
            for t in range(config.lm_sent_len):
                ...
                h_t_1_res = tf.matmul(x_t,self.Wres_1) + h_t_1_drop
                self.hidden1.append(h_t_1_drop)
                ##################  LSTM layer2  ########################
                if t==0:
                    self.H_t_2, self.state_t_2 = self.cell_2(h_t_1_res, self.initial_state_t_2)
                else:
                    self.H_t_2, self.state_t_2 = self.cell_2(h_t_1_res, self.state_t_2)
                ...
                h_t_2_res = tf.matmul(x_t,self.Wres_2) + h_t_2_drop
                self.hidden2.append(h_t_2_drop)
                ##################  LSTM layer3  ########################
                if t==0:
                    self.H_t_3, self.state_t_3 = self.cell_3(h_t_2_res, self.initial_state_t_3)
                else:
                    self.H_t_3, self.state_t_3 = self.cell_3(h_t_2_res, self.state_t_3)
        ...
        learning_rate = tf.train.exponential_decay(config.learning_rate,self.batch_num,1000,0.98)
    ...
    def initialize_weight(self):
        ...
        Wres_1 = self.weight_variable(shape=[self.word_embedding_size, H_dim[0]])
        Wres_2 = self.weight_variable(shape=[H_dim[0], H_dim[1]])


        return W_1, W_2, W_3, W_4, W_5, W_6, W_1_k, b_1_k, \
               W_1_l, b_1_l, W_3_k, b_3_k, W_3_l, b_3_l, W_5_k, b_5_k, W_5_l, b_5_l,Wres_1,Wres_2
