...
with graph.as_default(), tf.Session(config = config) as sess:
    ...
    # saver.restore(sess, os.path.join(cf.output_dir, cf.output_prefix,
    #                                  cf.dataname + str(0) + "_model_" + str(0) + ".ckpt"))
    ...
    for e in range(cf.epoch_size):
        ...
        for batch_id in batch_ids:
            ...
            # _, curr_cost, tm_cost, lm_cost, Theta, hidden_all = sess.run([DL.joint_train_step, DL.joint_Loss,DL.tm_Loss,DL.lm_Loss,[DL.theta_1C_HT,DL.theta_2C_HT,DL.theta_3C_HT], DL.hidden],
            #                         feed_dict={DL.Doc_input:d_train_batch, DL.phi_1:Phi[0], DL.phi_2:Phi[1],DL.phi_3:Phi[2],
            #                                    DL.pi_1:Pi[0], DL.pi_2:Pi[1] , DL.pi_3:Pi[2],
            #                                    DL.Sent_input: X_train_batch, DL.Sent_output: y_train_batch, DL.lm_mask: m_train_batch, DL.is_training: True })
            # Phi, Pi, NDot_Phi, NDot_Pi = update_Pi_Phi(d_train_batch, Phi, Pi, Theta, doc_num_batches, MBObserved, NDot_Phi, NDot_Pi)

            _, lm_cost,tm_like, Theta = sess.run([DL.joint_train_step, DL.lm_Loss,DL.L1,[DL.theta_1C_HT,DL.theta_2C_HT,DL.theta_3C_HT]],
                                    feed_dict={DL.Doc_input:d_train_batch, DL.phi_1:Phi[0], DL.phi_2:Phi[1],DL.phi_3:Phi[2],
                                               DL.pi_1:Pi[0], DL.pi_2:Pi[1] , DL.pi_3:Pi[2],
                                               DL.Sent_input: X_train_batch, DL.Sent_output: y_train_batch, DL.lm_mask: m_train_batch, DL.is_training: True })
            Phi, Pi, NDot_Phi, NDot_Pi = update_Pi_Phi(d_train_batch, Phi, Pi, Theta, doc_num_batches, MBObserved, NDot_Phi, NDot_Pi)
            ...
            if cf.save_model and batch_id%2000==0:
                ...
                for batch_id_t in batch_ids_test:
                    ...
                    X_test_batch, y_test_batch, d_test_batch, m_test_batch = get_batch_tm_lm_all(Doc_test, test_doc_bow,
                                                                                                     ...
                                                                                                     tm_ignore,
                                                                                                     cf.Batch_Size)
                    lm_cost_test, likelihood= sess.run(
                        [DL.lm_Loss, DL.L1],
                        feed_dict={DL.Doc_input: d_test_batch, DL.phi_1: Phi[0], DL.phi_2: Phi[1], DL.phi_3: Phi[2],
                                   ...
                                   DL.lm_mask: m_test_batch, DL.is_training: False})
                    test_lm_costs += lm_cost_test* cf.Batch_Size
                    test_tm_like += likelihood
                    test_lm_words += np.sum(m_test_batch)

                print("test lowerbound = %.3f" % (test_tm_costs / doc_num_batches_test))
                print("test lowerbound = %.3f" % (test_tm_like / doc_num_batches_test))

                print("test topic likelihood = %.3f" % (likelihood / doc_num_batches_test))
    ...
    pickle.dump([train_tm_cost,train_lm_perplexity,test_lm_perplexity], state_save)
    state_save.close()
