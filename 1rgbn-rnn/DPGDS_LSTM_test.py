...
#######################################LSTM_PARAM###############################################
        ...
        for j in range(config.sent_J):
            ...
            self.LB = self.LB + 1 * tf.reduce_sum(L1_1_t) + 0.1 * theta1_KL + 0.01 * theta2_KL + 0.001 * theta3_KL
            ...
            self.theta_2c = theta_2c/tf.maximum(tf.tile(tf.reshape(tf.reduce_max(theta_2c,axis=1),[self.Batch_Size,1]),[1,self.K_dim[1]]), config.real_min)
            self.theta_3c = theta_3c/tf.maximum(tf.tile(tf.reshape(tf.reduce_max(theta_3c,axis=1),[self.Batch_Size,1]),[1,self.K_dim[2]]), config.real_min)
            self.theta_1C_HT.append(self.theta_1c)
            self.theta_2C_HT.append(self.theta_2c)
            self.theta_3C_HT.append(self.theta_3c)
            ...
            for t in range(config.lm_sent_len):
                ...
                self.h_t_2_theta = self.GRU_theta_hidden(self.theta_2c, self.H_t_2, self.Uz2, self.Ur2, self.Uh2, self.Wz2, self.Wr2, self.Wh2, self.bz2, self.br2, self.bh2 )
                if self.is_training is True and config.lm_keep_prob < 1.0:
                    h_t_2_drop = tf.nn.dropout(self.h_t_2_theta, config.lm_keep_prob, seed=self.seed)
                else:
                    h_t_2_drop = self.h_t_2_theta
                ...
                self.h_t_3_theta = self.GRU_theta_hidden(self.theta_3c, self.H_t_3, self.Uz3, self.Ur3, self.Uh3, self.Wz3, self.Wr3, self.Wh3, self.bz3, self.br3, self.bh3 )
                if self.is_training is True and config.lm_keep_prob < 1.0:
                    h_t_3_drop = tf.nn.dropout(self.h_t_3_theta, config.lm_keep_prob, seed=self.seed)
                else:
                    h_t_3_drop = self.h_t_3_theta
        ...
        #### transpose hidden[240,8,600] to hidden_t[8,240,600]
        ...
        self.joint_Loss = self.tm_Loss + self.lm_Loss
        self.joint_Loss = self.tm_Loss + self.lm_Loss
