...
PHI_save = open('D:/PGBN_LSTM/DPGDS_LSTM_3layer/output-original/apnews-3-res/apnews-result.pckl', 'rb')
[train_tm_cost,train_lm_perplexity,test_lm_perplexity] = pickle.load( PHI_save )
PHI_save.close()
print(test_lm_perplexity)
num=len(test_lm_perplexity)
...
e = 5
batch_id_all = [2000,4000,6000,8000,10000,12000,14000]
...
with tf.Session() as sess:
    ...
    test_lm_perplexity = []
    for ba in range(7):
        batch_id = batch_id_all[ba]
        PHI_save = open(os.path.join(cf.output_dir, cf.output_path,
                                     cf.dataname + str(e) + '_Phi_Pi_hidden_Doc_' + str(batch_id) + '.pckl'), 'rb')
        [Phi, Pi] = pickle.load(PHI_save)
        PHI_save.close()
        ...
        # pickle.dump([test_lm_perplexity], state_save)
        # state_save.close()
    print(test_lm_perplexity)